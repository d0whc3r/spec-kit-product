...
| Data Design / Data Model | `erDiagram`                      | Only when the Data Design section is present.                                                                                                                                                               |
...
| Risks and Mitigations    | `quadrantChart`                  | Only when two or more risks are present AND they do not all land in one cell. Plot probability (x) by impact (y); Low near 0.2, Medium near 0.5, High near 0.85, and give shared cells a small offset only. |

The value gate and decision procedure above govern every conditional row in this table, including the `erDiagram` and the API `sequenceDiagram`: render them only when they clear the gate, never merely because the host section exists. A single entity with no relationship, or a single request and response, is a trivial shape and is omitted; the text block already states it.
...
5. Never emit an empty diagram, a diagram with placeholder labels (`ENTITY_A`, `Part name`), or a diagram for a section that was omitted.
...
| Diagrams and sections earn their place (value gate)    | Every conditional `mermaid` block and optional section passes the value gate: it reveals structure or a dimension the prose lacks, does not merely restate a list, does not duplicate another diagram or table, and is not a trivial shape. The Always Architectural Approach flowchart renders regardless but stays minimal and non-duplicative | No - requires judgment; remove any conditional block or section that only restates prose or duplicates another view |
