...
9. **`_(optional)_` is a marker, not title text.** In the template, a heading like `## Build Overview _(optional)_` uses `_(optional)_` only to flag that the section is optional. It is an authoring marker, never part of the title. When you keep the section, emit the heading clean (`## Build Overview`) with no `_(optional)_`. When the source has no real content that earns the section, omit the whole section, heading included. The string `_(optional)_`, and a bare `(optional)` suffix on any heading, must never appear in the generated document.

#### Section rules

- **Summary**: always present. One short paragraph (what is being built, who it is for, the main approach), then five labeled fields (Problem, For, Change, Quality bar, Constraints) derived from `plan.md` and `spec.md`. Keep each field to one sentence. Omit the Constraints field only when the source has none. No code, no file paths, no time estimates.
- **Goals and Non-Goals**: always present. Two sub-lists. `**Goals**`: three to six bullets, each a concrete observable outcome when complete. `**Non-goals**`: explicitly excluded capabilities with a one-phrase reason each. Always populate non-goals; if it feels empty, look harder at the plan's exclusions.
...
**Value gate (apply before emitting any conditional diagram).** A diagram earns its place only when it reveals structure that prose conveys poorly: connections between parts, ordering or dependencies, flows, state transitions, or a spread across two axes. Omit a conditional diagram when it would only re-list what the prose already enumerates, duplicate relationships another diagram or table in this document already shows, or collapse to a trivial shape (a single node, a straight chain with no branching, or a plot where every point lands in one cell). A reader must learn something from the diagram they could not get at a glance from the text. Prefer no diagram over a decorative one.
...
| --------------------- | --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Summary               | `journey`       | Only when `spec.md` describes a user-facing flow for the persona. Omit otherwise.                                                                                                                                                                                                                                    |
...
| Risks and Mitigations | `quadrantChart` | Only when two or more risks are present AND they do not all land in one cell. Plot probability (x) by impact (y); map Low near 0.2, Medium near 0.5, High near 0.85, and give shared cells a small offset only.                                                                                                      |
...
| Summary has context fields                                    | `## Summary` contains Problem, For, Change, Quality bar fields, non-placeholder; Constraints present unless the source has none                                                                                                                                                                     | Yes - derive from source                                                                                                                                |
...
| Diagrams and sections earn their place (value gate)           | Every conditional `mermaid` block passes the value gate: it reveals structure the prose lacks (connections, ordering, dependencies, flows, states, or a spread across axes), does not merely restate a list, does not duplicate another diagram or table, and is not a trivial shape                | No - requires judgment; remove any conditional block that only restates prose or duplicates another view                                                |
...
Sections populated: Summary, Goals and Non-Goals, Delivery Phases[, Build Overview][, Key Principles][, Key Decisions][, Risks and Mitigations][, Divergences and Edge Cases][, Validation][, Open Questions]
