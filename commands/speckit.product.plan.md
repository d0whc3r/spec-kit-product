...
| Diagrams present when section warrants                        | Each host section with source content has its `mermaid` diagram; no placeholder labels (`Part name`, `Phase 1 name`). The Delivery Phases `flowchart` is conditional: render only with two or more phases whose dependencies branch, and do not add it below that threshold                                                 | Yes - add diagram or fill from source (but never add the conditional Delivery Phases `flowchart` below the branching-phase threshold) |
