...
- On `yes`, continue. Each marker MUST appear as a bullet under "Open questions" in the Risks and Open Questions section of the generated `product/10-spec.md`. Never silently resolve a marker.
...
Sections populated: 7 mandatory[, Glossary][, Assumptions][, Positioning][, Go to Market and Rollout]
Open questions surfaced: <N>
Spec checklist: <PASSED>/<TOTAL> auto-validated[, <REMAINING> need manual review]
```

The `[, <REMAINING> need manual review]` segment is omitted when all spec items pass. `<N>` is the number of `[NEEDS CLARIFICATION]` markers surfaced into the Risks and Open Questions section.
