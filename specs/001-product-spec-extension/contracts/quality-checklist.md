# Contract: Product Spec Quality Checklist

This file is the **canonical product spec quality checklist**. Every product spec generated by `/speckit-product-spec` is paired with a `product/checklist.md` file (sibling of the generated `product/product-spec.md`) populated from this template. The checklist verifies that the generated artifact follows the canonical structure, the canonical style, and the canonical content rules.

The checklist is grouped into three blocks: structure, style, and content. A spec passes when every Required item is checked. Optional items only fail the checklist when their corresponding optional template section is present.
...
- The checklist file is created at the same time as the product spec.
- The generator pre fills metadata (feature name, date, links) but leaves every checkbox unchecked. The user or a reviewer ticks them after a manual or automated review pass.
- Re-running the command (overwrite path) re-creates the checklist with all boxes unchecked. Prior tick state is not preserved, since the underlying spec has changed.
