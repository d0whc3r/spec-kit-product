...
//
// Produces dist/product-<version>.zip containing only the extension's runtime
...
export async function buildZip() {
  ...
  const zipPath = path.join(distDir, `product-${version}.zip`);
  ...
  const stage = fs.mkdtempSync(path.join(os.tmpdir(), "product-zip-"));
  try {
    ...
    const verify = fs.mkdtempSync(path.join(os.tmpdir(), "product-verify-"));
    try {
